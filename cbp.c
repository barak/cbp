...
#include <c.h>
#include <ci.h>
#define myCSEARCH(str,var,tbl,msg)	/* int var;*/ ci_cstab str={&var,tbl,msg}
#include <del.h>
#include <libc.h>
...
{
  int index = sigmoid_table_resolution * z;

  if (index >= sigmoid_table_size) return 1.0;
  if (index < -sigmoid_table_size) return 0.0;
  return sigmoid_table[index+sigmoid_table_size];
}
...
net_kind net_type;
myCSEARCH(s_net_type, net_type, net_type_table, "network type");
...
typedef enum
{
  SIMPLE_SQUARED_ERROR	 	=0,
  MARGINATED_SQUARE_ERROR 	=1,
  CROSS_ENTROPHY 		=2,
  NORMALIZED_DISTRIBUTION 	=3,
  GAUSSIAN_VARIANCE 		=4,
} etype;
...
etype error_type;
myCSEARCH(s_error_type, error_type, etype_table, "error function");
...
inline bool net_traj_p(net_kind net_type)
{
  return net_type==NET_TRAJ_GUESS_Z || net_type==NET_TRAJ_BACKWARDS;
}
     ...
     char *file;
{
  ...
  FOREENV(e)
    {
      for (j=0, rp=e->z0_array; j<unit_count; j++, rp++)
...
void update_weights()
{
  ...
  cosine = dot/sqrt(w2*v2);
#else
  cosine = w2>0 && v2>0 ? dot/sqrt(w2*v2) : 1;
#endif

  write_real(stdout, " cos:", cosine);
  write_real(stdout, "|grad|:", sqrt(w2));
  write_real(stdout, "|v|:", sqrt(v2));
  ...
  if (display_Ev) {
    ...
    write_real(stdout, "v/E(v):", sqrt(v2)*(1-momentum)/sqrt(w2));
  }
  ...
  if (display_len_w) {
    ...
    write_real(stdout, "|w|:", sqrt(lenw2));
  }
...
REAL decay_error()
{
  ...
  if (net_traj_p(net_type) && T_decay)
     ...
     char *arglist;
{
  ...
  printf("\tmean: %g, stdev: %g\n", mean, sqrt(total2/conn_count - mean*mean));
  printf("\tmin: %g, max: %g, mean abs %g.\n", mmin, mmax, totala/conn_count);
}
     ...
     const char *arglist;
{
  ...
  printf("Randomizing Weights, excursion %f", excursion);
     ...
     pattern pat;
{
  ...
  if (error_type == NORMALIZED_DISTRIBUTION)
    {
      FOROUTS(pu,rp,pat)
	{
	  REAL r = *rp;
  ...
  FOROUTS(pu,rp,pat)
      ...
      if (r != DONTCARE)
	{
	  ...
	  switch (error_type)
	    {
	    case SIMPLE_SQUARED_ERROR:
	      u_external_dE_dy(u) = e = y-r;
	      total_e += e*e/2;
	      break;

	    case MARGINATED_SQUARE_ERROR:
	      u_external_dE_dy(u) =
		e = 
		  (r > 0.5 ?
		...
		if (y==0.0) {
		  y = 0.00001;
		  fprintf(stderr,
			  "bumping zero output %d for pattern %d\n",
			  u, NUMBER_PAT(pat));
		  fprintf(stderr,
			  "\t(u_sig_x(%d) == %f, u_I(%d) == %f)\n",
			  u, u_sig_x(u), u, u_I(u));
		...
		total_e += e*e/variance + log(std) + SQRT_PI_O2;
	      }
	      break;
     ...
     FILE *fd;
{
  ...
  if (display_B)
    printf("B %g |B| %g ", B, sqrt(B2/B2_norm2));
     ...
     REAL total_B;
{
  ...
  if (_del_>1) return;
  total_e += backward_part(e, true, (FILE *)NULL);
  if (_del_>1) return;

  if (display_B && net_type==NET_TRAJ_GUESS_Z) printf("\n");

  if (display_cases && !net_traj_p(net_type))
    {
...
void initialize()
{
...

  {
    ...
    sprintf(a,"%f %ld", 1.0, time((long *)NULL));
    randomize_weights(a);
  }
  ...
  back_criterion = 1e-6;
  delta_t = .1;
  error_type = SIMPLE_SQUARED_ERROR;
  square_margin = 0.2;
  weight_decay = 0;
  T_decay = 0;
     ...
     char **argv;
{
  ...
  printf("Starting cbp, a neural network simulator.\n");
  printf(" Copyright 1988-1993 Barak Pearlmutter, released under GNU Public License v2.\n");
  printf(" Use at your own risk; no warranty expressed or implied.\n");
